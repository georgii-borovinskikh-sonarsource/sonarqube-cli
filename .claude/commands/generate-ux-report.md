...
This command takes 1–3 minutes depending on whether the CLI binary needs to be compiled. It is completely self-contained: it spins up fake servers internally and cleans them up afterwards. It is safe to run at any time and does not modify any source files (only `docs/cli-output-report.md` and potentially `build-scripts/docs/generate-ux-report.ts` if new scenarios are discovered).
...
Good:
- `Read tests/integration/specs/ux-report.test.ts` — see what's already covered
...
For each approved gap, add a `uxIt()` entry to the appropriate `uxDescribe()` block in `build-scripts/docs/generate-ux-report.ts`.
...
- After editing, run `bun x prettier --write build-scripts/docs/generate-ux-report.ts` to format
